...
**基準時刻は「引き継ぎ・申し送りを記録」ボタンを押した時刻**。実際の交代は定刻ちょうどとは
限らず、また営業時間自体も変わりうるため、**コード側は固定の交代時刻を一切持たない**。
営業時間が変わっても改修は不要。
...
- **引き継ぎ記録が無いときは、何も新着にしない**。
  以前は「直近に過ぎた固定時刻（18時）」にフォールバックしていたが、昼間に画面を見ると
  基準が前日の分になり、**その日の朝に入った予約まで新着扱い**になってしまった
  （ユーザーからの指摘で発覚）。基準は押した時刻だけにする方が、見え方が予測できる
- 引き継ぎがまだ無い状態で申し送りだけを書いた場合は、`handed_over_at` に**その時点**を入れる。
...
**貼り付いた（stuck）間だけ幅を狭くする**: 本来の位置にある間（ページ最上部で
スクロールしていない状態）は他のパネルと同じ幅のまま、実際にヘッダー直下へ
固定されて浮いて見える間だけ `max-width: 26rem` に狭める。

判定は `position: sticky` の状態をCSSだけでは取れないため、直前に高さ0の
目印（`#dutyStickySentinel`）を置き、それが画面外に出たかを
...
他のパネルと同じ幅のまま画面の大半を占めてしまう。閉じておけば、実際に必要になったとき
（＝スクロールして貼り付いたとき）だけ狭い形で現れる。
