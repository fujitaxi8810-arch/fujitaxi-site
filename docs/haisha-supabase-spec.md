...
- 同じ電話番号を持つ、まだ使われていない「新規追加」行が今回のCSVにあれば
  **「変更あり」候補**（予約時刻が変わった可能性）。無ければ**「キャンセル」候補**
- 貪欲マッチ（先に見つかった1件を消費して次には回さない）なので、
  同じ電話番号で複数件が同時に消えても、新規行の数だけしか「変更あり」にならない。
  余りは「キャンセル」扱いになる（1対1を超える曖昧な対応付けはしない）
...
これは自動判定ではあるが確定前に必ず人の目を通す設計にしている。特に「変更あり」の
電話番号マッチングは、施設の代表番号などを複数人が共有しているケースで誤結合しうるため、
確定前のプレビューで対象者を見てから判断できるようにした。
