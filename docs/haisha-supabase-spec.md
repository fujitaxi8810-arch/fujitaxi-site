...
ユーザー要望：「交流の送迎を追加したい。シフトに印で対応」。それまで「交流」は
`/shift` に該当するコード（文字）が無く、`/haisha` の乗務割パネルで**日ごとに手で追加**
するしかなかった（§5.5参照）。月次シフト作成の時点では反映できなかった。

**新しいコードは増やさなかった**。交流の送迎はどのシフトコード（①普通番／③遅番等）の日でも
担当しうるため、コードの選択肢を1つ増やす形（貸切のように）にすると、その日の本来の
勤務種別と二者択一になってしまい表現できない。代わりに、**電話番と同じ「コードとは別に
立てる印」**として実装した。

```sql
alter table shifts add column exchange_duty boolean not null default false;
```

- `/shift`：セルを右クリック（またはタップ長押し相当のコード選択ポップアップ）で開くコード
  選択画面に、「電話番を担当」と並べて「交流の送迎を担当」チェックボックスを追加。
  電話番と同じく、**先にコードが入っていないと付けられない**（コードが空の状態で交流だけ
  付ける状況が無いため）。セルの見た目は、指定のあった「チェックすると下に棒線」に合わせ、
  ボタン下端に `box-shadow: inset` の下線（`.sf-exchange-mark`、色は`/haisha`の「新着」表示と
  同じ `#0e7490` に揃えた）を重ねる。border-bottomではなくbox-shadowにしたのは、
  ボタンの枠線（1px）を太らせてレイアウトをずらさないため
- コードを空にする（クリア／サイクルで「－」に戻す）と、電話番と同様に交流の印も一緒に外れる
  （コードが無い日に印だけ残る状態を防ぐ）
- 「前月の内容をコピー」は `exchange_duty` も含めてそのままコピーする
- 変更履歴からの「元に戻す」は、コード・電話番のみ過去の値に戻し、**交流の印は今の値を保つ**
  （履歴テーブル `shift_history` 側に `exchange_duty` の新旧を記録する仕組みが無いため。
  必要になれば別途トリガーの拡張が要る）
- `/haisha` の乗務割パネル：`DUTY_GROUPS` の「交流」に `shiftFlagKey: 'exchangeDuty'` を追加。
  コード一致（`shiftKey`）では拾えないため、`dutyRows` 全体を `d.exchangeDuty` で絞り込む
  専用の経路を用意した（`fromShiftByCode` と `fromShiftByFlag` を合流させる）。
  日ごとの追加（`dailyKey: 'exchange'`、×で外せる分）は従来通り併用できる

**マイグレーション未実行のままデプロイした場合の影響（重要）**：`upsertShift` が
`exchange_duty` を含む形に変わっているため、列が無い状態でコード選択・電話番トグル・
前月コピー・履歴の「元に戻す」など**シフトへの書き込み全般が失敗する**（読み取り専用の
表示は影響なし）。**必ず上記SQLを先に実行してからデプロイすること**。
...
## 5.92 「全体で見る」タブ（2026-08-21 追加）

ユーザー指摘：「日別ボードは1日分でも縦に長く、スクロールしないと全件を把握するのに時間がかかる」。
当初は別ページ・独自レイアウト（旧スプレッドシート「配車確認表」に似せた乗務割グリッド＋
電話番号付きの一覧）を検討したが、ユーザーの意向で**仕様（列・担当者割当・電話番号を出さない等）は
日別ボードと完全に同一**とし、違いは「複数列に分けてスクロールを減らす」ことだけに絞った。

- 3つ目のタブ「全体で見る」として追加。`showTab()` を3択に拡張した以外、
  **既存の日別ボード・予約一覧のコードは一切変更していない**（ユーザーからの明示の指示）
- 表示は行ではなくカード（`.ha-all-card`）。CSS Grid の `repeat(auto-fit, minmax(19rem, 1fr))` で
  画面幅に応じて自動的に列数が変わる（1280pxで3列、500pxで1列）。**1時間経過後の自動非表示はしない**
  （「全部の予約を見る」ためのタブなので、ここだけは常に全件を出す）
- `buildStatusSelect` / `buildStaffSelect` / `applyStaffColor` / `pickupText` / `destinationText` /
  `isNewSinceHandover` / `buildNewMark` / `buildSupersededNote` / `buildSupersededFromNote` /
  `openEdit` は日別ボードと共通の関数をそのまま呼ぶ。独自に書いたのは、カード形式に合わせた
  `applyAllCardStatus`（`applyRowStatus`のカード版）と`renderAllView`本体のみ
- **連動**：担当者・状態・チェックをどちらのタブで操作しても、もう一方のタブに切り替えた瞬間に
  最新の状態が反映される（要望：「日別と全体は連動するように」）。実現方法は単純で、
  `showTab()` がタブを切り替えるたびに対応するデータを**必ず読み直す**ようにした
  （`board`→`loadBoard()`、`all`→`loadAllView()`。`list`は元々こうなっていたので、
  同じパターンに揃えただけ）。リアルタイム同期ではなく「切り替えるたびに読み直す」方式
- 印刷は日別ボードの印刷CSSと同じ考え方（`.kt-panel__tools`等の非表示ルールは共通で効く）で、
  カード用のルールだけ追加した
- 本番データ（8/21・19件）で動作確認：カード表示、複数台予約、変更ありの矢印、
  レスポンシブ（1280px→3列／500px→1列）、担当者割当の双方向連動（ボード⇄全体）を確認。
  テストで割り当てた担当者は確認後に必ず未割当へ戻している

### 修正①：1時間経過での自動非表示を日別ボードと揃える（2026-08-21）

初回実装では「全部の予約を見る」ことを優先し、全体タブだけ時間経過での自動非表示をしない仕様に
していたが、ユーザーから「仕様は日別ボードと同じにしてほしい」と明確な指示があり修正。
`isPastReservation` / `HIDE_AFTER_MINUTES` は日別ボードと共通の関数・定数をそのまま使い、
`showPastAll` という専用のstateと「経過分も表示」ボタンを別途持たせる形で、
ボード側のコード・状態には一切触れずに同じ挙動を再現した。

### 修正②：カードが大きすぎて結局スクロールが要る問題（2026-08-21）

複数列にしても、1枚のカードが「時刻＋状態セレクト／お客様名＋担当者セレクト／迎車場所メモ／
済チェック＋編集」の4行構成で、`.ha-select`（タッチ操作前提でmin-height 2.75rem）を2個含んでいたため、
19件で4〜5行×120px超＝スクロールが必要な高さになっていた。ユーザーからの「小さくなってもいいので
全体を確認できるように」という指示で、レイアウトを作り直した：

- カード内に限って `.ha-select` を大幅に小型化（min-height 2.75rem→1.3rem、font-size 1rem→0.64rem）。
  `.ha-select.ha-status` 側の個別指定と詳細度が同じ（class×2）ため、ソース順で後に置くことで上書きした
- 4行→3行に再構成：①時刻＋お客様名（文字のみ）②迎車場所→行き先メモ（小さく）
  ③状態・担当者・済・編集を1行に集約
- `.ha-linkbtn`（min-height 2.5rem）・`.ha-check` も同様にカード内だけ縮小
- 文字サイズはページ下部の「文字サイズ」切替（`document.documentElement.style.fontSize`をrootに設定）が
  そのまま効く。カードの数値はすべてrem単位にしてあるため、標準表示で小さくても
  大きくしたい人は既存のズーム機能で調整できる
- 結果：1枚あたり約120px→67pxに圧縮。19件・6列で必要な高さも478px→349pxまで縮小

### 修正③：行優先ではなく列優先（1列ごとに時系列）に並べる（2026-08-21）

CSS Gridの `auto-fit` に任せると、カードは行優先（左から右へ埋め、右端まで来たら次の行）で
並ぶため、1列の中では時系列にならない（例：1行目が7:30/7:40/8:00、2行目が8:10/8:20/8:20…と
横に読まないと時系列にならず、最初のミニ mockup で見せていた「新聞のカラム落とし」とは
実は違う並びになっていた）。ユーザー指摘で気づき、列優先に作り直した。

- `computeAllColumns()`：画面幅と現在のズーム倍率（`documentElement`のfont-size）から
  置ける列数を計算する。CSSの `minmax(10.5rem, ...)` と同じ基準をpx換算して揃えている
- `renderAllView()`：列数ぶんの `.ha-all-col`（縦積みのブロック）をJSで作り、
  時系列のまま「1列目の上から下、2列目の上から下…」の順に振り分ける
  （`perCol = Math.ceil(件数 / 列数)`で分割）。件数が少ない日は、置けるはずの列数より
  少ない列しか作らない（余った列が空のまま横に伸びるのを防ぐ）
- `.ha-all-grid` は `display:flex`（列を横に並べるだけ）、`.ha-all-col` が
  `display:flex; flex-direction:column`（列の中を縦に積む）
  という2段構成にした（CSS Gridの列優先auto-flowは行数を固定しないと機能しないため、
  件数が毎日変わるこの用途には向かなかった）
- 画面のリサイズで列数が変わったときだけ組み直す `resize` リスナーを追加
  （文字サイズズームで列数が変わる場合にも対応）

### 修正④：件数が少ない日に1件ずつバラバラの列になる（2026-08-21）

上記③の直後、実際に「要対応3件」の日で確認したところ、**3列×1件ずつ**というバラバラな
並びになってしまった（ユーザー指摘：「縦に並んでません」）。原因は
`perCol = Math.ceil(件数 / 置ける最大列数)` という式。件数が少ない日は
（例：3件・最大6列）`ceil(3/6)=1`になり、1列1件で列を作ってしまっていた。

`TARGET_ROWS_PER_COL = 4` を導入し、**まず1列に最低4件詰めてから**、それでも
収まらない分だけ次の列を使う順番に直した：`colCount = min(置ける最大列数, ceil(件数/4))`。
3件なら1列（3件が縦に並ぶ）、19件なら5列（4/4/4/4/3、③と同じ結果を維持）。

### 修正⑤：カードではなく表形式に作り直す（2026-08-21）

ユーザーから改めて旧スプレッドシート「配車確認表」の画像イメージを示され、
「カードではなく表形式（行・列）にしたい」と明確な要望があった（電話番号は
引き続き出さない方針のまま）。枠で囲んだ`.ha-all-card`ブロックを複数列に並べる方式から、
**独立した小さな`<table>`を複数、横に並べる**方式へ全面的に作り直した：

- 各列＝1つの`<table class="ha-all-table">`。ヘッダーは日別ボードと同じ `headerRow()` を再利用
  （時刻／状態／担当者／お客様名／場所・メモ／済／編集の7列。迎車場所と行き先メモは
  幅を抑えるため1セルにまとめている）
- 状態・担当者は日別ボードと同じ `<select>`（`buildStatusSelect`/`buildStaffSelect`）を
  そのままセルに入れている。行の背景色（未割当＝オレンジ系／変更あり＝黄色系／
  キャンセル＝グレー打ち消し線）も日別ボードと同じ考え方で`<tr>`にクラスを付け、
  `<td>`側に効かせている（`tr`単体への背景指定はブラウザ実装差があるため）
- `computeAllColumns()` の列幅の目安をカード時代の10.5remから、表の`min-width: 15rem`に
  合わせて変更。これをせず放置すると、実際に置ける表の数とJSが計算する列数がずれ、
  表がはみ出す／隙間が空きすぎる、という不具合になるところだった
- 印刷CSSも `.ha-all-card` 系のセレクタを `.ha-all-table`/`.ha-all-name`/`.ha-all-place` に
  更新（旧クラス名を放置すると印刷時だけ黒文字化ルールが効かなくなるところだった）

### 修正⑥：お客様名の後ろに場所を続けて大きく＋乗務割も追加（2026-08-21）

続けて2つの要望：①場所・メモをお客様名の後ろに続けて大きい文字にしたい、②日別ボードにある
乗務割パネルを全体タブにも出してほしい。

**①名前＋場所の統合**：「場所・メモ」列を廃止し、お客様名セルの2行目として場所・メモを
表示するよう変更（`ha-all-name`セル内に`<div>`を2つ）。列が1つ減った分、文字も
0.6rem→0.72remへ大きくした。

**②乗務割の共有**：`dutyPanel`はDOM上1つしか無く日別ボードと共有のため、
`dutyDateKey()`を「今どのタブを見ているか」で日付を切り替えるよう拡張
（全体タブを見ているときは`allDatePick`の日付、それ以外は`datePick`の日付）。
`showTab()`で`dutyPanel`の非表示条件を`!board`から`!(board || all)`に変更し、
`loadAllView()`でも`fetchShiftsByDate`/`fetchDailyDuties`を取得して`renderDuty()`を呼ぶようにした。

**この2つの変更で発覚した表幅の不具合（重要）**：`computeAllColumns()`が使っていた
「1列あたりの目安幅」を決め打ちのrem値にしていたが、実際に必要な幅（時刻・状態・担当者・
お客様名(+場所)・済・編集の6列ぶん）は大きく食い違っており、**列数の見積もりを大きく外し、
CSSのflex-wrapで折り返した分だけ縦に何倍にも積み上がる**という重大な不具合になっていた
（一時、19件が1〜2列に収まらず1000px近く縦に伸びる状態になった）。加えて、
`.ha-all-table`に`flex:1`（伸びて空きを埋める設定）を使っていたため、列数がズレて少ない列しか
横に並ばないと、その少ない表が大きく伸び、次にその「伸びた幅」を測ってしまい、
ますます列数を減らす方向に暴走する悪循環も発生した。

対処：
- `.ha-all-table`を`flex:1`（伸縮あり）から`flex:0 0 auto`（内容なりの自然な幅のまま）に変更
- `computeAllColumns()`は決め打ちのrem値をやめ、**直前に実際に描画された表の幅を測って**次回の
  計算に使う（`lastAllTableWidthPx`）。初回描画時だけ暫定値（27rem）を使い、1回目の描画直後に
  実測値で2回目を描き直して収束させる（`loadAllView()`内）
- 「変更あり」の注記が長い文章になると、それ単体で表の幅を押し広げる原因にもなっていたため、
  該当セル・注記に`max-width`と省略表示を追加。詳細な変更点一覧（`.ha-superseded-diff`）は
  全体タブでは非表示にした（詳細は日別ボード・予約一覧で確認する前提。矢印自体は出る）
- 実機相当（1920×1080）で確認したところ、余白+33pxで収まる。1280×800のような小さめの
  画面では表の実質幅（約430px/表）の都合で2列までしか横に並ばず、多少のスクロールが残る
  （サイトの`.kintai-wrap`系コンテナに1280px前後の上限があるため、画面をさらに広げても
  横方向の余地がそれ以上増えないという既存の制約による）

### 修正⑦：複数列に分けるのをやめ、横長の1枚表に作り直す（2026-08-21）

ユーザー要望：「横長にしてください幅を広げて一行に一つの予定をつくる」。度重なる列数計算の
不具合（修正⑤・⑥）を踏まえ、**列に分ける方式そのものをやめる**方向に転換した。

- `computeAllColumns()` とその呼び出し（列数・1列あたりの件数の計算、`.ha-all-table`を
  複数生成して振り分ける処理）を全て削除。表は**1つだけ**、`rows`をそのまま順に1行ずつ
  積むだけのシンプルな作りに戻した
- 幅の見積もり違いによる折り返し暴走・flex伸縮の悪循環（修正⑥参照）は、
  分割そのものをやめたことで根本的に解消
- 列も日別ボードと同じ8列（時刻・状態・担当者・お客様名・迎車場所・行き先メモ・済・編集）に戻し
  （修正⑥で「お客様名」に統合していた場所・メモは別列に戻す）、文字サイズも
  幅の制約が無くなった分、日別ボードに近い大きさまで戻した（0.55〜0.78rem→0.85〜0.95rem）
- 「変更あり」の注記・変更点一覧（`.ha-superseded-diff`）も、幅を気にする必要が無くなったので
  全体タブでの非表示指定を解除した（日別ボードと同じくフル表示に戻る）
- 横スクロールが必要な場合に備え、`.ha-all-grid`に`overflow-x: auto`を付けている

同時にタブの名称・並び順もユーザー要望で変更：「全体で見る」→「全体配車ボード」
（「日別の配車ボード」と対になる名前）、並びは「日別の配車ボード／全体配車ボード／予約一覧」の順。

### 修正⑧：行の高さを詰めて、より多くの件数を一度に見せる（2026-08-21）

ユーザー要望：「高さを狭くしてもっと全体がうつるようにして30件MAX」。`.ha-select`
（タッチ操作前提でmin-height 2.75rem）・`.ha-linkbtn`（同2.5rem）が1行の高さ（実測58px）の
主な原因だったため、この表の中に限って項目の高さを2段階で詰めた：

- 1回目：td/thの上下パディングを縮め、セレクト・編集ボタンの `min-height` を1.9remに
  → 58px→約37px
- 2回目：さらにパディングを詰め、`min-height` を1.5rem・チェックボックスも1remまで縮小
  → 約37px→約28px（当初の半分以下）

30件をこの高さで並べると約1430px相当になり、実機（1920×1080）でも完全にはゼロスクロールに
ならない（ヘッダー・タブ・乗務割パネル等で先頭が553px下がっている分の影響が大きい）。
これ以上セレクトを縮めると文字が読みづらくなるため、可読性とのバランスでこの値とした。

### 修正⑨：迎車場所・行き先メモが省略されて切れる（2026-08-21）

`.ha-all-place`（迎車場所・行き先メモの列）に `max-width: 16rem` の省略表示を付けていたが、
実データでは48セル中28セルが実際に切れていた（最長35文字）。ユーザー指摘で修正。

`max-width`・`overflow: hidden`・`text-overflow: ellipsis` を外し、内容の長さなりに
表を横へ伸ばすようにした。縦の行の高さ（修正⑧で詰めた分）を崩さないよう
`white-space: nowrap` は残す。表が画面幅を超える場合は `.ha-all-grid` の
`overflow-x: auto`（元から設定済み）で横スクロールする。実データで確認したところ、
切れているセルは0件になった（表幅1777px・画面幅1099pxのときは横スクロールが発生する）。

### 修正⑩：フォーカス枠が隣の行にはみ出す（2026-08-21）

サイト共通のキーボードフォーカス枠（`::focus-visible { outline: 2px solid var(--fuji-yellow);
outline-offset: 3px; }`、`global.css`）は通常サイズの入力欄向けで、修正⑧で行を28pxまで
詰めたこの表では余白が足りず、選択欄にフォーカスすると枠が隣の行にはみ出して見えていた
（ユーザーがスクリーンショットで指摘）。`.ha-all-table` の中の `.ha-select` /
`.ha-check` / `.ha-linkbtn--icon` に限って `outline-offset: 0px` を上書きし、枠が
要素にぴったり沿うようにした（フォーカス表示自体は残しているのでアクセシビリティは維持）。
サイト全体のフォーカス枠には影響しない。

### 修正⑫：担当者を割り当てた行が読めなくなる（2026-08-21）

ユーザーがスクリーンショットで報告：「全体配車ボードで担当者を選択して決めると（その行が）真っ暗になる」。
詳しく聞くと「行自体が状態と担当者以外暗くなる」とのことで、状態・担当者のセレクトだけ無事、
という手がかりから原因を特定できた。

`.ha-all-table td` に**背景色の指定が無かった**。ページ（`.kt-panel`）はダークテーマの黒背景で、
`未割当`・`変更あり`・`キャンセル`の各状態だけ`td`に明るい背景色（オレンジ系・黄系・グレー系）を
付けていたが、**どれにも当てはまらない「通常状態」の行**（＝担当者を割り当てた直後、まさに
一番よく発生する状態）は背景指定が無いまま、ページの黒背景がそのまま透けて見えていた。
文字色は`color:#111`（ほぼ黒）を常に指定していたため、黒背景に黒文字が重なって見えなくなっていた。
状態・担当者のセレクトだけ無事だったのは、`.ha-select`自体が独自に白背景を持っていたため。

`.ha-all-table td { background: #fff; }` を追加し、通常状態の行にも白背景を敷いた
（未割当等の色はこの白背景の上から`td`単位で上書きされる形になる）。

### 追加：日別ボード・全体配車ボードの自動更新（2026-08-21）

ユーザー要望：「自動でリロードとかしてくれますか」。複数人で同じ画面を開けっぱなしにしていても、
他の人のCSV取り込み・担当者割当が自動で反映されるようにしたい、とのこと。

- `loadBoard()` / `loadAllView()` を、タブが表示されている間だけ **30秒おき**に呼び直す
  （既存の「1時間経過での非表示」用60秒タイマーとは別に追加。あちらは件数チェックのみで軽いが、
  こちらは実際にデータを取り直す）
- ちょうど`<select>`等を操作している最中に描き直すと選択中の項目が消えてしまうため
  （`hiddenRows`の60秒タイマーに元々あった配慮と同じ理由）、`document.activeElement`が
  フォーム要素（select/input/textarea）のときは今回のタイマーをスキップする
  （`isInteractingWithForm()`）

### 修正⑪：ニューステッカーがhidden属性を付けても消えていなかった（2026-08-21）

`hideNav`実装直後、ユーザーから「NEWSの部分は隠すことはできないですか」と指摘。調べたところ、
`.news-ticker`要素自体は`hidden`属性が付いていた（HTML上は正しい）が、`global.css`の
`.news-ticker { display: flex; ... }` という通常のルールが、ブラウザ標準の
`[hidden] { display: none }`（ユーザーエージェントスタイルシート）を**詳細度に関わらず**
上書きしてしまい、実際には表示されたままになっていた（作者スタイルシートの通常ルールは、
詳細度の勝敗以前に、ユーザーエージェントの通常ルールより常に優先されるため）。

`global.css`に`.news-ticker[hidden] { display: none; }`（詳細度0,2,0）を追加し、
`.news-ticker{display:flex}`（詳細度0,1,0）に確実に勝つようにして解決。
`.nav`側は`display`を独自指定していなかったため、この問題は起きていなかった
（標準の`[hidden]`がそのまま効いていた）。公開サイト（ホームページ等）では
`hidden`属性が付かないため、ニューステッカーは変わらず表示される。
